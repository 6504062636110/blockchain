...
const carbonCreditRate: { [key: string]: number } = { PET: 6, PE: 7, PP: 5, PC: 8, PVC: 4 };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen p-4 pt-20">
      <div className="bg-orange-300 p-6 rounded-2xl shadow-lg w-[400px]">
            ...
            <ul className="mt-2 space-y-2">
              {list.map((item, index) => (
                <li key={index} className="flex justify-between items-center border-b pb-1">
                  <span>{item.type} - {item.amount} piece(s)</span>
                  <button
        ...
        <div className="mt-4 flex justify-center">
          <Button className="bg-slate-600 text-white px-6 py-2 rounded-lg" onClick={() => setIsOpen(true)}>
            Confirm Order
          </Button>
        </div>
      </div>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
        <div className="bg-white p-6 rounded-lg shadow-xl w-[400px]">
          <Dialog.Title className="text-lg font-semibold">Confirm your transaction</Dialog.Title>
          <hr className="my-3" />
          ...
          <div className="text-sm">
            ...
            {list.map((item, index) => (
              <div key={index} className="grid grid-cols-3 mt-2">
                ...
                <span className="text-right font-bold">{item.amount * (carbonCreditRate[item.type] || 0)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-between items-center text-lg font-semibold">
            ...
            <span className="text-xl text-green-600">{totalCarbonCredit} credits</span>
          </div>

          {/* ปุ่มใน Modal */}
          <div className="flex justify-between mt-6">
            <Button className="bg-gray-800 text-white px-4 py-2 rounded-md" onClick={() => setIsOpen(false)}>
              Cancel
            </Button>
            <Button
